...
public class IAdiviserPourRegner extends IA{
	...
	Coup CoupSuivant(ArrayList<Coup> ListeCoups, Plateau P) {
		...
		// au moins soit présent sur ces sous plateaux (pour pouvoir jouer)
		...
		if (!plateauScindable(P)){
			...
			if (!listeCourante.isEmpty()){
				...
				// et on oublie pas de traduire le coup trouve dans les coordonnées
				// du grand plateau
				...
				if (plateauCourantI<repetitionsI-1){
					...
					if (plateauCourantJ<repetitionsJ-1){
						...
						// des coups jouables à la "frontiere" des sous plateaux
	...
	boolean plateauScindable(Plateau P){
		...
		for (int I = 0; I<repetitionsI*PasI; I=I+PasI){
			for (int J = 0; J<repetitionsJ*PasJ; J=J+PasJ){
				// I et J sont les coordonnees de la première case du sous tableau
				...
				for (int i = I; i<I+PasI; i++){
					for (int j = J; j<J+PasJ; j++){
						if (etat[i][j] == Case.Trou){
							trou = true;
							// On vérifie que chaque sous plateau possede 
							// au moins un trou
						}
					}
				}
